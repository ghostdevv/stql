// Based on MIT Licensed code by Marijn Haverbeke
// https://github.com/cookshack/codemirror-ansi/blob/04448a0c0caa5c3cb2dd3b87795b3cbbb9450e93/LICENCE
...
function makeDec(attr) {
	let css;

	css = '';
	if (attr.fg) css += ' cm-ansi-' + clrs[attr.fg].name;
	if (attr.bg) css += ' cm-ansi-' + clrs[attr.bg - 10].name + '-bg';
	if (attr.bold) css += ' cm-ansi-bold';
	return Decoration.mark({ attributes: { class: css } });
}

function clr(name, color) {
	let css;
	...
	return {
		...
		boldBg: Decoration.mark({
			attributes: { class: css + '-bg cm-ansi-bold' },
		}),
	};
}

function clr2(i, name, color, bright) {
...
export function ansi(options: Options = { step: null }): Extension[] {
	return [
		baseTheme,
...
// ESC[1;32m
// ESC[1;32;43m
csRe = /\x1B\[([0-9]*)((?:;[0-9]+)*)m/dg; // (?: ) is non capturing group

function decoLine(builder, cache, line) {
	...
	function push(attr) {
		...
		if (attr.from == undefined)
			...
			attr.cache = 0;
		} else if (attr.bg && attr.fg) attr.dec = makeDec(attr);
		else if (attr.bg) attr.dec = bgDec(attr.bg, attr.bold);
		else if (attr.fg)
			attr.dec = attr.bold ? clrs[attr.fg].bold : clrs[attr.fg].norm;
		else if (attr.bold) attr.dec = clrs[1].bold;
		ranges.push(attr);
	}

	function add(from, len /* of marker */, to, num, init) {
		...
		if (1) push({ from: from, to: from + len, hide: 1 });
		// reset
		if (num == 0) {
			fg = 0;
			bg = 0;
		...
		if ([1, 22].includes(num)) {
			if (num == 22) {
				...
				else push({ bold: 0, fg: 0, bg: 0 }); // dummy, for cache
			}
			if (num == 1) {
				// bold
				boldOn();
	...
	function addGroup(line, start, end, group) {
		let slice, num;
	...
	ranges = [];
	if (line.number > 0) hit = cache[line.number - 1];
	fg = hit?.fg || 0;
	bg = hit?.bg || 0;
	bold = hit?.bold || 0;
	if (hit) {
		...
		if (hit.fg) addAttr(line, 0, 0, hit.fg, 1);
		else if (hit.bg) addAttr(line, 0, 0, hit.bg, 1);
		else if (hit.bold) addAttr(line, 0, 0, 1, 1);
	}
	csRe.lastIndex = 0;
	...
	matches.forEach((match) => {
		...
		if (match.indices.length > 2) {
			...
			if (group2[0] == group2[1]) return;
			line.text
				.slice(group2[0], group2[1])
				.split(';')
				.forEach((attr) => {
					if (attr.length) addAttr(line, start, end, attr);
				});
		}
	});
...
const showAnsi = ViewPlugin.fromClass(
	class {
		constructor(view) {
